...
wxJSONValue::Cat( const wxString& str )
{
    ...
    if ( data->m_type == wxJSONTYPE_STRING )  {
        wxJSONRefData* data2 = COW();
        wxJSON_ASSERT(data2);
        data2->m_valString.append( str );
        r = true;
    }
    return r;
}
...
wxJSONValue::Cat( const wxMemoryBuffer& buff )
{
    ...
    if ( data->m_type == wxJSONTYPE_MEMORYBUFF )  {
        wxJSONRefData* data2 = COW();
        wxJSON_ASSERT( data2 );
        data2->m_memBuff->AppendData( buff.GetData(), buff.GetDataLen());
        r = true;
    }
    return r;
}
...
wxJSONValue::GetInfo() const
{
    ...
    s.Printf( _T("Object: Type=%s Size=%d comments=%d\n"),
            wxJSONValue::TypeToString( data->m_type ).c_str(),
            Size(), (int)data->m_comments.GetCount() );
#endif
    if ( data->m_type == wxJSONTYPE_OBJECT ) {
...
{
    int siz = (int)comments.GetCount(); int r = 0;
    for ( int i = 0; i < siz; i++ ) {
...
wxJSONValue::GetCommentCount() const
{
    ...
    int d = (int)data->m_comments.GetCount();
...
/* static */
void wxJSONValue::SetToNull(wxJSONValue &value )
{
	wxJSONValue n;
	value = n;
}
